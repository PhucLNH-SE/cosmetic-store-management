...
public class ManageBrandPageVM : BaseViewModel
{
    ...
    private void Delete(object? parameter)
    {
        ...
        ApplyFilter();
        statusmessage = "Delete brand successfully. Brand is now inactive.";
    }
    ...
    private string ValidateInput()
    {
        ...
        if (textboxitem.BrandName.Trim().Length > 100)
        {
            return "Brand name must be <= 100 characters.";
        }

        if (!string.IsNullOrWhiteSpace(textboxitem.Country) && textboxitem.Country.Trim().Length > 100)
        {
            return "Country must be <= 100 characters.";
        }
